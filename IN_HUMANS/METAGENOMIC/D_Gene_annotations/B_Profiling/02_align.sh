...
## Variables
catdir=/scratch/<USERS>/<Project_scratch>/catalogue/derepcat
indir=/scratch/<USERS>/<Project_scratch>/bw_cleaned_reads
outdir=/scratch/<USERS>/<Project_scratch>/catalogue/alignment
...
bwa mem -a -t 8 ${catdir}/Gene_catalog_derep.fna ${indir}/${sample}/${sample}_bowtie2_R1_001.fastq.gz \
    | samtools view -F 4 -bh - > ${outdir}/${sample}/${sample}_r1.bam

echo "Running BWA MEM for reverse : " ${sample}
bwa mem -a -t 8 ${catdir}/Gene_catalog_derep.fna ${indir}/${sample}/${sample}_bowtie2_R2_001.fastq.gz \
